...
Sub Class_Globals
	...
	Private xui As XUI
	Private lstEquipos As List
	Private mEquipos As Map 
	Private nf  As String = "equipos.txt"
	Private strCarpeta  As String 
End Sub
...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	strCarpeta =  File.DirTemp  & "tema17\"
	Log(strCarpeta)
	lstEquipos.Initialize
	cargarEquipos
	
	'Para cada ítem de la lista, copiarlo en un array y
	'después lo ponemos en un mapa
	mEquipos.Initialize
	For i = 0 To lstEquipos.Size - 1
		Private arr() As String
		arr = lstEquipos.Get(i)
		mEquipos.Put(arr(0), arr(1))
	Next
	
	File.WriteMap(strCarpeta, nf, mEquipos)
	
	'Copiar todo el mapa en una única cadena
	Private strEquipos As String
	For Each clave  As  String In  mEquipos.Keys
		strEquipos = strEquipos & clave  & " " & mEquipos.Get(clave) & CRLF
	Next
	
	'Escribir cadena de texto al fichero
	File.WriteString(strCarpeta, "equipos2.txt", strEquipos)
	Log(strEquipos)
...
Private Sub cargarEquipos
	...
	lstEquipos.Add(Array As String ("Sao Paolo","Few teams in the world could match the success found by Brazilian giants Sao Paulo in the early 1990s. Under legendary head coach Tele Santana, Sao Paulo reached the pinnacle of their powers during the 1993 season. "))
	lstEquipos.Add(Array As String ("Dynamo Kiev","Dynamo Kiev have long been the dominant force in Ukranian football, but in the 1970s and 80s, they were the dominant force in all of Soviet football, ruling the league."))
End Sub
